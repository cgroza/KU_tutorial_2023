...
We can view an example graph alignment as a ~json~ record with the command:
#+BEGIN_SRC sh :exports code
vg view -aj /data/NA12878.chr22.gam | head -n1 | jq . | less
#+END_SRC
#+begin_quote
Use the up and down arrow keys to navigate, press ~q~ to exit.
#+end_quote
Use the up and down arrow keys to navigate, press ~q~ to exit.
There are many similarities with the BAM format. For example, there are fields
for the read name and its mate, and a mapping quality field. The ~path~ field is
analogous to the chromosome field but is more complex. As you can see, it is an
enumeration of nodes instead of a chromosome and the alignments describe the
path in the pangenome on which the read lies. The edit fields are analogous to
the CIGAR string in BAMs.
